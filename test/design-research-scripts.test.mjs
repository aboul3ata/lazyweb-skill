...
test("fill-report: produces a gate-passing report with correct escaping", async () => {
  ...
  assert.ok(!html.includes("picsum.photos"), "filled report must not retain template placeholder images");
  assert.ok(!html.includes("data-ex="), "filled report must not retain example markers");
});
