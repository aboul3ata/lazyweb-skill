...
4. pass `image_url` to `lazyweb_growth_report` (action=start report_args) / `render` / `compare_image` / `find_similar` (the low-level `synthesize` / `start_mockup` chain is deprecated — current clients get a `tool_deprecated` rejection)
