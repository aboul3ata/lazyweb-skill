...
## Affected skills (audit 2026-06-25)

There are **two** byte-leak paths, and most skills hit at least one:
1. **Input image** — user screenshot → `compare_image` inline base64 / `signup_design_run({image_b64})` / `synthesize`+mockup.
2. **Render assets** — local screenshots → `render_report`/`publish_report` as `assets:[{b64}]` (agent emits base64 at render; the `REPORT_TOO_LARGE` / ~26 MB-render-502 failure mode). Broader — 6 skills.

| skill | input-image path | render-assets b64 | corruption risk | fixed by this PR | needs migration |
|---|---|---|---|---|---|
| optimize-paywall | script + token upload | no | med | ✅ behavior only | ✅ (drop token) |
| design-improve | **inline base64** | ✅ | high | ❌ | ✅ (emits bytes twice) |
| deep-design-research | **inline base64** (fallback) | ✅ control + ≤4 prototypes | high | ❌ | ✅ |
| paywall-cta | **inline base64** | ✅ | high | ❌ | ✅ |
| optimize-sign-up | **inline base64** (`signup_design_run`) | no (server renders) | high | ❌ | ✅ |
| lite-design-research | render-assets only | ✅ current-state | med | ❌ | ✅ |
| design-brainstorm | render-assets only | ✅ current-state | high | ❌ | ✅ |
| ab-test-research | URL-only input | ✅ assets | med (high 502) | ❌ | ✅ |
| design-best-practices | none | no | none | n/a | ❌ no images |
| quick-search | none | no | none | n/a | ❌ no images |

**8 skills need migration; this PR implements 1 (behavior only).** The decision covers all; the implementation lands skill-by-skill.

## Rollout — per-skill checklist

**Phase 0 — server (unblocks everything):**
- [ ] `lazyweb_request_image_upload({mime_type, byte_size?})` → `{upload_url (presigned PUT, ~10m TTL), image_url}`, behind MCP auth.
- [ ] Confirm `compare_image` / `find_similar` / `synthesize` / `start_mockup` / `render_report` all accept `image_url` for every image arg (input AND render assets).
- [ ] (interim, optional) `lazyweb_issue_cli_token` for token self-heal.
- [ ] Metrics: daily image-send volume + daily fail% on lazylanding `/metrics` (this lets us confirm the drop).

**Phase 1 — migrate input-image paths** (replace inline base64 / token-script with: request upload → `curl -T` → pass `image_url`):
- [ ] design-improve (`compare_image`) — highest impact (51% fail today)
- [ ] paywall-cta (`compare_image`)
- [ ] optimize-sign-up (`signup_design_run` → accept `image_url`)
- [ ] deep-design-research (`compare_image` fallback)
- [ ] optimize-paywall (replace `optimize_paywall.py` token upload; retire the token)

**Phase 2 — migrate render-assets paths** (pass `assets` by `image_url` the server fetches, not `{b64}`):
- [ ] design-improve, deep-design-research, design-brainstorm, lite-design-research, paywall-cta, ab-test-research

**Phase 3 — cleanup:**
- [ ] Remove `~/.lazyweb/lazyweb_mcp_token` dependency + `ensure_token`/`install-token` from the install path once no skill needs it.
- [ ] Drop the "fail clearly / self-heal" interim language from SKILL.md once upload is tokenless.
